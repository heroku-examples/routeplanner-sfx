    ...
    when(mockContext.getOrg())
        .then(
            i1 -> {
              ...
              when(mockOrg.getDataApi().query("SELECT Id, Name FROM Account"))
                  .then(
                      i2 -> {
                        ...
                        when(firstRecord.getStringField("Name"))
                            .thenReturn(Optional.of("Account One, inc."));

                        Record secondRecord = mock(Record.class);
                        when(secondRecord.getStringField("Id"))
                            .thenReturn(Optional.of("6003000000D8cuIQAA"));
                        when(secondRecord.getStringField("Name"))
                            .thenReturn(Optional.of("Account Two, inc."));

                        when(mockResult.getRecords())
                            .thenReturn(Arrays.asList(firstRecord, secondRecord));
